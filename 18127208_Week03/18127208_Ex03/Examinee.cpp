...
float CalcScoreOfNS(Examinee obj) {
    return obj.physic + obj.chemistry + obj.biology;
}
float CalcScoreOfSS(Examinee obj) {
    return obj.history + obj.geography + obj.civic_education;
}
float CalcSumOfScore(Examinee obj) {
    obj.natural_science = CalcScoreOfNS(obj);
    obj.social_science = CalcScoreOfSS(obj);
    return obj.math + obj.literature + obj.foreign_language + obj.natural_science + obj.social_science;
}
void WriteSumOfScore(const std::vector<Examinee>& examinee_list, std::string out_file_name) {
    std::ofstream fout;
    fout.open(out_file_name);
    for (auto& x : examinee_list) {
        fout << x.id << " " << CalcSumOfScore(x) << "\n";
    }
    fout.close();
}